...
#     driver = serializers.IntegerField()
    ...
    def create(self, validated_data):
        ...
        if not validate_hour.validateHour(hour):
            raise serializers.ValidationError("Hour field have incorrect format, should be hh:mm, in 24h, in range 8:00 to 18:00")
        validate_calendar = self.validateCalendar(id_driver, date_order, hour)
        if not validate_calendar:
            raise serializers.ValidationError("Driver is busy in selected hour, please select other")
        find = FindDriver()
    ...
    def validateCalendar(self, id_driver, date_order, hour):
        query = Order.objects.filter(driver=id_driver).filter(date_order=date_order).filter(hour=hour)
        if len(query)==0:
            return True
        return False
